...
def make_user(client, db_session):
    ...
    def _make(email=None, password="test123456", username=None,
              role="motorcyclist", verified=False):
        counter["n"] += 1
        ...
        assert response.status_code == 201, (
            f"make_user: register failed: {response.get_json()}"
        )
...
def auth_headers(client, make_user):
    ...
    assert response.status_code == 200, (
        f"auth_headers: login failed: {response.get_json()}"
    )
