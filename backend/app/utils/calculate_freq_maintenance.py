...
def calculate_maintenance_freq(moto_id, user_id):
    ...
    try:
        ...
        if not maintenances:
            return {"total_maintenances": 0, "month_maintenances": 0, "chart_data": []}
        ...
        month_maintenances = len(
            [m for m in maintenances if m.date and m.date >= month_start]
        )
        ...
        for i in range(11, -1, -1):
            ...
            chart_data.append(
                {"month": month_date.strftime("%Y-%m"), "value": month_total}
            )
