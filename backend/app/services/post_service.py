...
class PostService:
    ...
    def create_post(author_id: int, content: str, image_file=None) -> Post:
        ...
        post = Post(author_id=author_id, content=content.strip(), image=image_path)
        db.session.add(post)
        db.session.commit()
    ...
    @staticmethod
    def get_posts(
        page=1, per_page=20, user_id=None, current_user_id=None, include_comments=False
    ):
    ...
    def update_post(
        ...
        elif delete_image and post.image:
            from app.utils.files import delete_file
